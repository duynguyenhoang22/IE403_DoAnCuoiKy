...
TEENCODE_STYLES: list[str] = [
    # Level 0 – Không obfuscation
    "Viết bình thường, có dấu, formal như tin nhắn ngân hàng thật "
    "(KHÔNG dùng leet, KHÔNG viết tắt, KHÔNG ký tự đặc biệt)",

    # Level 1 – Leet nhẹ (hiện tại: entry 1 & 5 cover tốt)
    "Thay e=3, a=4, o=0, i=1 và chèn dấu chấm/gạch ngang xen kẽ "
    "(ví dụ: T.u.y.3.n, S-h-0-p-e-e)",

    # Level 2 – Leet nặng tên riêng (hiện tại: entry 2 cover)
    "Dùng j thay gi, f thay ph, w thay qu, z thay d "
    "(ví dụ: th0ng b4o vj fhat, jao luu za.lo)",

    # Level 3 – Mixed special chars (hiện tại: entry 3 & 4 gần đúng)
    "Chèn ký tự đặc biệt @, #, !, *, ^ liên tục vào từ khóa nhạy cảm "
    "để lách bộ lọc",

    # Level 4 – Dot/dash insertion từng ký tự
    "Tách từng ký tự bằng dấu chấm hoặc gạch ngang "
    "(ví dụ: A-M-A-Z-O-N, t.u.y.e.n.d.u.n.g, C-h-u-c-m-u-n-g)",

    # Level 5 – Extreme noise
    "Chèn ký tự nhiễu ngẫu nhiên xen kẽ từng âm tiết, mix tiếng Anh/Việt "
    "vô trật tự, dùng homoglyph: 'l' thay 'I', '0' thay 'O', ký tự Unicode "
    "trông giống Latin (ví dụ: tORKiM, GR N'ha'nL,jen:Qu.a;HangN)",
]
...
def _prompt_banking(brand: str, style: str, size: int) -> str:
...
ĐẶC TRƯNG CỦA LOẠI NÀY (bắt buộc áp dụng):
  ...
  - Cú pháp chuẩn ngân hàng giả: "[{brand.upper()}] Tài khoản..." hoặc "{brand} trân trọng..."
...
def _prompt_debt_threat(brand: str, style: str, size: int) -> str:
...
def _prompt_bhxh(brand: str, style: str, size: int) -> str:
...
Tổ chức / chương trình bị giả mạo: {brand}
...
def _prompt_job_scam(brand: str, style: str, size: int) -> str:
...
def _prompt_gambling(brand: str, style: str, size: int) -> str:
...
def _prompt_govt_fake(brand: str, style: str, size: int) -> str:
...
ĐẶC TRƯNG BẮT BUỘC:
  - Mở đầu bằng tên cơ quan: "{brand}: ..." hoặc "[{brand.upper()}]" hoặc "Bộ ... xin thông báo"
  ...
  - Domain giả: dichvucong[s].top / phatnguoi.xyz / hoanthue-tncn.vip / vnta-gov.cc / {brand_slug}-gov.top
...
def _prompt_sensitive(brand: str, style: str, size: int) -> str:
...
Khu vực / nền tảng liên hệ: {brand}  (ví dụ: "HCM – Telegram", "HN – Zalo")
...
def _prompt_crypto(brand: str, style: str, size: int) -> str:
...
def build_prompt(category: str, brand: str, style: str, size: int) -> str:
    """Dispatch tới category-specific prompt template (8 templates từ Sprint 2)."""
    fn = _PROMPT_DISPATCH.get(category)
    if fn is None:
        raise ValueError(f"Category không xác định: {category!r}. "
                         f"Hợp lệ: {list(_PROMPT_DISPATCH.keys())}")
    return fn(brand=brand, style=style, size=size)
